#Requires -PSEdition Core
$ErrorActionPreference = 'Stop'

$Environ = Get-Content -Path .env | ConvertFrom-StringData
$ResourceGroup = $Environ.RESOURCE_GROUP
$Location = $Environ.LOCATION
$VMName = $Environ.VM_NAME

$VirtualMachineParameters = @{
  "adminUsername" = @{ "value" = [string]$Environ.ADMIN_USERNAME };
  "adminPassword" = @{ "value" = [string]$Environ.ADMIN_PASSWORD };
  "OSVersion"     = @{ "value" = [string]$Environ.OS_VERSION };
  "vmSize"        = @{ "value" = [string]$Environ.VM_SIZE };
  "vmName"        = @{ "value" = [string]$VMName };
  "computerName"  = @{ "value" = [string]$Environ.COMPUTER_NAME };
  "diskSizeGB"    = @{ "value" = [int]$Environ.DISK_SIZE_GB };
} |
ConvertTo-Json -Compress |
ForEach-Object { $_ -replace '"', '\"' }

az group create `
  --name $ResourceGroup `
  --location $Location `
  --output table

$result = az deployment group create `
  --resource-group $ResourceGroup `
  --template-file ./CreateWindowsServerVirtualMachine.bicep `
  --parameters $VirtualMachineParameters `
  --output json | ConvertFrom-Json

$storageAccountName = $result.properties.outputs.userStorageAccountName.value
$storageContainerName = $result.properties.outputs.scriptsContainerName.value
az storage blob upload `
  --account-name $storageAccountName `
  --container-name $storageContainerName `
  --name "Install-JapaneseLanguagePack.ps1" `
  --file .\Install-JapaneseLanguagePack.ps1 `
  --auth-mode key `
  --output table

$scriptUri = "https://$storageAccountName.blob.core.windows.net/$storageContainerName/Install-JapaneseLanguagePack.ps1"
az vm run-command invoke `
  --resource-group $ResourceGroup `
  --name $VMName `
  --command-id RunPowerShellScript `
  --scripts "Invoke-RestMethod -Uri $scriptUri | Invoke-Expression" `
  --output table
