...
- [ ] Task: Prettierの構成確認と調整
    - [ ] `.prettierrc` または `package.json` 内の設定を確認し、必要に応じてプロジェクトのスタイルガイドに合わせる。
    - [ ] `.prettierignore` を確認し、ビルド成果物などが除外されているか確認する。
- [ ] Task: 全ファイルへのフォーマット適用
    - [ ] プロジェクト全体のファイルを対象に Prettier を実行し、コードスタイルを統一する。
- [ ] Task: フォーマット済みの検証
    - [ ] `pnpm format` (チェックモード) を実行し、すべてのファイルが正しくフォーマットされていることを確認する。
